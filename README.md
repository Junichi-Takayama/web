...
### 3. リポジトリ分離・チーム開発保護（1クライアント＝1リポジトリ運用）
- **親リポジトリ（`web`）**: 高山様の社内管理用（事業計画書・見積基準・AIルール・各社ディレクション議事録）。外部パートナーやクライアントの招待は厳禁。
- **制作コード（`clients/*/site/`）**: 案件ごとに独立した GitHub リポジトリとして運用。親の `.gitignore` により自動除外され、Gitの干渉を防ぎます。
- **外部パートナー連携**: 外部コーダーやデザイナーには、該当クライアントの `site` リポジトリのみアクセス権を付与（Google Driveのフォルダ単体共有と同じ安全設計）。
